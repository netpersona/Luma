...
export default function EpubReader() {
    ...
    try {
      ...
      rendition.on("rendered", (section: any, view: any) => {
        ...
        if (currentContents && currentContents.document) {
          ...
          // Handle text selection for highlighting - trigger immediately on mouseup
          doc.addEventListener('mouseup', () => {
            ...
            setTimeout(() => {
              ...
              if (text && text.length > 0) {
                // Get the CFI range for the selection using epub.js contents API
                try {
                  const range = selection.getRangeAt(0);
                  // Use the contents object's cfiFromRange method
                  ...
                  // Silently fail - fallback "selected" event will handle it
                }
              }
            }, 10);
          });
          ...
          doc.addEventListener('click', (e: any) => {
            // Don't trigger navigation if text is selected
            const selection = win.getSelection();
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (renditionRef.current) {
        // Pass undefined to use auto-sizing based on container
        renditionRef.current.resize(undefined as any, undefined as any);
      }
    }, 100);
  ...
  // Separate useEffect to render highlights when they change
  useEffect(() => {
  ...
  return (
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      ...
      >
        {/* The actual epub content - click navigation is handled inside the iframe */}
        <div 
