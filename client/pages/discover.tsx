...
export default function Discover() {
  ...
  const googleBooksApiKeySetting = settings?.find(s => s.key === "user:default:googleBooksApiKey");
  ...
  const addRecommendationMutation = useMutation({
    mutationFn: async (rec: RecommendationItem) => {
      ...
      const searchTerm = rec.isbn13 || rec.isbn10 || rec.title;
                  ...
                  {recommendations.map((rec) => (
                    <Card key={rec.googleBooksId} className="flex flex-col overflow-hidden" data-testid={`card-recommendation-${rec.googleBooksId}`}>
                      <div className="aspect-[2/3] relative bg-muted overflow-hidden">
                        ...
                        <div className={`absolute inset-0 flex items-center justify-center ${rec.thumbnail ? 'hidden' : ''}`}>
