...
import { BookOpen, ArrowLeft, Clock, Calendar, Tag, Building2, BookMarked, MessageSquare, Download, Send, FolderPlus, Check, ImageIcon, Image, Pencil, Search, Loader2, Trash2, Sparkles } from "lucide-react";
...
export default function BookDetail() {
  ...
  const fetchSeriesMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", `/api/books/${bookId}/fetch-series`, {
        autoUpdate: true,
      });
      return response.json();
    },
    onSuccess: (data: { success: boolean; metadata: { seriesName: string | null; seriesIndex: number | null; confidence: string; source: string }; book: BookWithProgress }) => {
      queryClient.invalidateQueries({ queryKey: ["/api/books", bookId] });
      queryClient.invalidateQueries({ queryKey: ["/api/books"] });
      ...
      if (data.metadata.seriesName) {
        ...
        toast({
          ...
          description: "Could not find series information for this book. You can add it manually via Edit Metadata.",
        });
      }
    },
    onError: (error: any) => {
          ...
          <TabsContent value="details" className="mt-6">
            {book.description && (
              <div className="mb-6" data-surface="content">
                ...
                <div 
                  className="prose prose-sm dark:prose-invert max-w-none text-muted-foreground first-letter:text-5xl first-letter:font-serif first-letter:font-bold first-letter:mr-1 first-letter:float-left first-letter:leading-none first-letter:text-foreground [&_p]:mb-4 [&_strong]:font-semibold [&_em]:italic"
