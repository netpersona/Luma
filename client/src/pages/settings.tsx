...
import { Palette, Check, Download, Bell, BellOff, User, Settings2, Trash2, AlertTriangle, RefreshCw, HardDrive, BookMarked, Search } from "lucide-react";
...
interface SeriesFetchResult {
  total: number;
  updated: number;
  failed: number;
  details: Array<{ title: string; series?: string; seriesIndex?: number; error?: string }>;
}
...
export default function Settings() {
  ...
  // Bulk series fetch mutation
  const bulkSeriesFetchMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/library/fetch-all-series");
      return response.json() as Promise<SeriesFetchResult>;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/books"] });
      queryClient.invalidateQueries({ queryKey: ["/api/audiobooks"] });
      toast({
        title: "Series fetch complete",
        description: `Found series info for ${data.updated} out of ${data.total} items.`,
      });
    },
    onError: (error: any) => {
      toast({
        title: "Series fetch failed",
        description: error.message || "Failed to fetch series metadata.",
        variant: "destructive",
      });
    },
  });
          ...
          <TabsContent value="application" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Automatically detect and fill in series information for books and audiobooks
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="text-sm text-muted-foreground">
                  This will search Open Library and analyze title patterns to find series 
                  information for all books and audiobooks that don't have series data.
                  This process may take a while for large libraries.
                </div>
                <Button
                  onClick={() => bulkSeriesFetchMutation.mutate()}
                  disabled={bulkSeriesFetchMutation.isPending}
                  variant="outline"
                  className="w-full"
                  data-testid="button-fetch-all-series"
                >
                  {bulkSeriesFetchMutation.isPending ? (
                    <>
                    ...
                    <>
                      ...
                      Find Series Info for All Items
                    </>
                  )}
                </Button>
                {bulkSeriesFetchMutation.isPending && (
                  <Alert>
                    ...
                    <AlertDescription>
                      Searching for series information... This may take several minutes for large libraries.
                    </AlertDescription>
                  </Alert>
                )}
              </CardContent>
